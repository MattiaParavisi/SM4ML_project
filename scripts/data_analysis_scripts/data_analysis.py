...
dir = os.path.join(".", "Cats_resized")
if not os.path.exists(dir):
    os.mkdir(dir)

dir = os.path.join(".", "Dogs_resized")
if not os.path.exists(dir):
    os.mkdir(dir)

dir = os.path.join(".","..","..","CatsDogs_resized")
if not os.path.exists(dir):
    os.mkdir(dir)

dir = os.path.join(".","..","..","CatsDogs_resized","Dogs_resized")
if not os.path.exists(dir):
    os.mkdir(dir)

dir = os.path.join(".","..","..","CatsDogs_resized","Cats_resized")
if not os.path.exists(dir):
    os.mkdir(dir)
...
for fldr in os.listdir(folder_path):
    ...
    for filee in os.listdir(sub_folder_path):
        ...
        if file_path.endswith(".jpg"):
            img = Image.open(file_path)
            img = img.convert('RGB')
            img_resized = tf.image.resize(img, (baseheight, basewidth), preserve_aspect_ratio=False)
            #0 -> .
            #1 -> ..
            #2 -> ..
            #3 -> CatsDogs
            ...
            tf.keras.utils.save_img(f'./{arr_specs[4]}_resized/{arr_specs[5]}', img_resized.numpy(), file_format='JPEG', data_format='channels_last', scale=True)

from_directory = "./Dogs_resized"
to_directory = "./../../CatsDogs_resized/Dogs_resized"
copy_tree(from_directory, to_directory)

from_directory = "./Cats_resized"
to_directory = "./../../CatsDogs_resized/Cats_resized"
copy_tree(from_directory, to_directory)
